...
class JumlahOverview extends BaseWidget
{
    ...
    protected function getCardStats(string $statName): ?Stat
    {
        return match($statName) {
            ...
            'total_generus'     => Stat::make('Generus', Generus::owned()->count()),
            'total_caberawit'   => Stat::make('Caberawit', Generus::owned()->where('jenis_data', 'CBRWT')->count()),
            'total_mudamudi'    => Stat::make('Muda-Mudi', Generus::owned()->where('jenis_data', 'MM')->count()),
            'total_pra_remaja'  => Stat::make('Pra Remaja', Generus::owned()->where('kategori', 'PRA_REMAJA')->count()),
            'total_remaja'      => Stat::make('Remaja', Generus::owned()->where('kategori', 'REMAJA')->count()),
            'total_pra_nikah'   => Stat::make('Pra Nikah', Generus::owned()->where('kategori', 'PRA_NIKAH')->count()),
